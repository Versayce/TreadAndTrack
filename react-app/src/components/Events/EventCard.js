    ...
    const eventCards = events?.map((event) => {
        return (
            <EventCards onClick={() => setActiveEventPage(event.id)} key={event.id}>
                <h1>{event?.name}</h1>
                {event.images[0]?.imageUrl != "" ? <img alt='eventimg' src={`${event.images[0]?.imageUrl}`} onError={e => {e.currentTarget.src = "/images/placeholderImage.png";}}/> : <img alt='placeholder' src='/images/placeholderImage.png'/>}
                <EventLocation>
